 ...
 * Esta función ejecuta un MYSQL Query, puede devolver 1 dato (one) o muchos datos (all).
 */
 function newPostData($query, $params, $type) {
	 try {
		 $con= getConnection();
		 $data= $con->prepare($query);
		 ...
		 for ($i = 0; $i < count($params); $i ++) {
			 $key = $i + 1;
			 print_r($key, $params[$i]);
			 $data->bindParam($key, $params[$i]);
		 }
...
function postData($query, $type){
	try {
		...
		return "Error: Los datos no están disponibles. ERRORCODE: " . $e->getMessage();
	}
}
?>
